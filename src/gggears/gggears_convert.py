...
class GearToNurbs():
    def __init__(self,
                 ...
                 convertmethod='fast'):
        self.params = params
        self.gear = gg.InvoluteGear(params=self.params)
        self.n_points_hz = n_points_hz
        self.n_points_vert = n_points_vert
        self.oversamp_ratio = oversampling_ratio
        self.n_z_tweens = int(np.ceil((self.n_points_vert-2) * \
                                      self.oversamp_ratio) + 2)
    ...
    def generate_nurbs(self):
        ...
        for gear_stack_loc in self.gear_stacks:
            ...
            for gearprofile in gear_stack_loc:
                ...
                tooth_nurb = crv.convert_curve_nurbezier(gearprofile.tooth_curve,
                ...
                NurbsConv = crv.NURBSCurve(*[rd_nurb,
    ...
    def generate_surface_points_sides(self, method='fast'):
        ...
        for ii in range(len(self.params.z_vals)-1):
            ...
            points_combined = np.concatenate([points_asd,
                                              weights_asd[:,:,np.newaxis]],
                                            axis=2)
            if method == 'fast':
                sol2,points2,weights2 = self.solve_surface_fast(points_combined,
                                                                n_points_vert=self.n_points_vert)
            else:
                sol2,points2,weights2,t = self.solve_surface(points_combined,
                                                            n_points_vert=self.n_points_vert,
                                                            t_weight=0.01)

            surface_data.append(NurbSurfaceData(points=points2,
                                                 weights=weights2,
                                                 knots=stack[0].knots[:]))
